...
gapminder %>%
    ...
    summarise(AvggdpPercap = mean(gdpPercap, na.rm = T)) %>%
    ungroup() %>% 
    ggplot(mapping = aes(x = year, y = AvggdpPercap, colour = continent)) +
    geom_line(na.rm = T) +
    geom_point(alpha = 0.5, size = 2.5, na.rm = T) +
    labs(
        x = "Year",
        y = "Mean GDP (in dollar)",
        title = "AvG GDP is been increasing over the years"
        ) +
    theme_minimal()+
  ggsave(filename="AvgGDP.png")
```
...
gapminder %>%
    group_by(country) %>%
  mutate(meanGDPcountry=mean(gdpPercap, na.rm = T)) %>%
  ungroup() %>% 
 mutate(incomeGroup= cut_number(meanGDPcountry,n=5)) %>% 
  group_by(incomeGroup,year) %>% 
  summarise(meanGDP=mean(gdpPercap,na.rm=T))
    ungroup() %>% 
    ggplot(mapping = aes(x = year, y = meanGDP, colour = incomeGroup)) +
    geom_line(na.rm = T) +
    geom_point(alpha = 0.5, size = 2.5, na.rm = T) +
    labs(
        x = "Year",
