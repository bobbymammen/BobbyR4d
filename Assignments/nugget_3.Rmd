...
```
12. Why aren't `selectedCarrier` and `flightCarrier` identical? Think about the data
structure that `select()` returns compared to that for `pull()`/`$`
##when you use select you get a table itself of selectedCarrier which is of 1 column vs  flightCarrier is just a variable
...
flights %>% 
filter(dep_delay>10) %>% 
  count(origin,sort=TRUE,name="Count of flights")
...
```{r}
flights %>% 
recode(origin,JFK = "John F Kennedy")
```
...
flights %>% 
 ...
 mutate (timeMadeUp=dep_delay-arr_delay) %>% 
  group_by(carrier) %>% 
    summarise(meandeparturedelays = mean(timeMadeUp, na.rm = T)) %>% 
ggplot(mapping = aes(x = carrier, y = meandeparturedelays)) +
  geom_col(mapping = aes(group = carrier)) 
...
```{r}
flights %>% 
    ggplot(mapping = aes(x =distance, y = arr_delay)) +
...
ggplot(mapping=aes(dep_delay))+
  ...
  facet_grid(carrier ~ origin,scales="free_x",space="free")+
  ggsave("Facet_Nugget.png")
```
...
ggplot(mapping = aes(x = carrier, y = meandeparturedelays)) +
  geom_col(mapping = aes(group = carrier)) +
labs(x = "Carrier Name",
         y = "TIme made up while in air",
         title = "Most flights make time while in air",subtitle = "A chart I made for the daily tasks")+
  theme_economist() +
